    ...
    class YWMMQ_Tags_Bulk_Ops {
        ...
        public function output() {
            ...
            $table->options = array(
                ...
                'count_table'      => '( SELECT COUNT(*) FROM wp_terms a INNER JOIN wp_term_taxonomy b ON a.term_id = b.term_id INNER JOIN wp_woocommerce_termmeta c ON c.woocommerce_term_id = a.term_id WHERE b.taxonomy = "product_tag" AND ( c.meta_key = "_ywmmq_tag_exclusion" OR c.meta_key = "_ywmmq_tag_quantity_limit_override" OR c.meta_key = "_ywmmq_tag_value_limit_override") AND c.meta_value = "yes" GROUP BY a.term_id ) AS count_table',
                'count_where'      => '',
                'key_column'       => 'ID',
                'view_columns'     => array(
                ...
                'custom_columns'   => array(
                    ...
                    'column_override_qty' => function ( $item, $me ) {
                        ...
                        if ( $item['override_qty'] == 'yes' ) {
                            $class  = 'show';
                            $tip    = __( 'Yes', 'yith-woocommerce-minimum-maximum-quantity' );
                            $min    = get_woocommerce_term_meta( $item['ID'], '_ywmmq_tag_minimum_quantity', true );
                            $max    = get_woocommerce_term_meta( $item['ID'], '_ywmmq_tag_maximum_quantity', true );
                    ...
                    'column_override_val' => function ( $item, $me ) {
                        ...
                        if ( $item['override_val'] == 'yes' ) {
                            $class  = 'show';
                            $tip    = __( 'Yes', 'yith-woocommerce-minimum-maximum-quantity' );
                            $min    = get_woocommerce_term_meta( $item['ID'], '_ywmmq_tag_minimum_value', true );
                            $max    = get_woocommerce_term_meta( $item['ID'], '_ywmmq_tag_maximum_value', true );
                            $limits = sprintf( __( 'Min.: %d - Max.: %d', 'yith-woocommerce-minimum-maximum-quantity' ), $min, $max );
                        }
                        else {
                            $class  = 'hide';
                        ...
                        'function_delete' => function () {
                            ...
                            if ( !empty( $ids ) ) {
                                $wpdb->query( "UPDATE {$wpdb->prefix}woocommerce_termmeta
                                           SET meta_value='no'
                                           WHERE ( meta_key = '_ywmmq_tag_exclusion' OR meta_key = '_ywmmq_tag_quantity_limit_override' OR meta_key = '_ywmmq_tag_value_limit_override') AND woocommerce_term_id IN ( $ids )"
                                );
                            }
                        }
                    )
            ...
            if ( !empty( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], basename( __FILE__ ) ) ) {
                ...
                if ( $item_valid !== true ) {
                    ...
                    foreach ( $tag_ids as $tag_id ) {
                        update_woocommerce_term_meta( $tag_id, '_ywmmq_tag_exclusion', $exclusion );
                        update_woocommerce_term_meta( $tag_id, '_ywmmq_tag_quantity_limit_override', $override_qty );
                        update_woocommerce_term_meta( $tag_id, '_ywmmq_tag_minimum_quantity', $minimum_qty );
                        update_woocommerce_term_meta( $tag_id, '_ywmmq_tag_maximum_quantity', $maximum_qty );
                        update_woocommerce_term_meta( $tag_id, '_ywmmq_tag_value_limit_override', $override_val );
                        update_woocommerce_term_meta( $tag_id, '_ywmmq_tag_minimum_value', $minimum_val );
                        update_woocommerce_term_meta( $tag_id, '_ywmmq_tag_maximum_value', $maximum_val );
                    }
            ...
            if ( isset( $_GET['id'] ) && !empty( $_GET['action'] ) && ( 'edit' == $_GET['action'] ) ) {
                ...
                $item = array(
                    'ID'           => $_GET['id'],
                    'excluded'     => get_woocommerce_term_meta( $_GET['id'], '_ywmmq_tag_exclusion', true ),
                    'override_qty' => get_woocommerce_term_meta( $_GET['id'], '_ywmmq_tag_quantity_limit_override', true ),
                    'minimum_qty'  => get_woocommerce_term_meta( $_GET['id'], '_ywmmq_tag_minimum_quantity', true ),
                    'maximum_qty'  => get_woocommerce_term_meta( $_GET['id'], '_ywmmq_tag_maximum_quantity', true ),
                    'override_val' => get_woocommerce_term_meta( $_GET['id'], '_ywmmq_tag_value_limit_override', true ),
                    'minimum_val'  => get_woocommerce_term_meta( $_GET['id'], '_ywmmq_tag_minimum_value', true ),
                    'maximum_val'  => get_woocommerce_term_meta( $_GET['id'], '_ywmmq_tag_maximum_value', true )
                );
                            ...
                            <tr valign="top">
                                ...
                                <td class="forminp">
                                    ...
                                    <input
                                        ...
                                        id="tag_ids"
                                        name="tag_ids"
                                        data-placeholder="<?php _e( 'Search for a tag&hellip;', 'yith-woocommerce-minimum-maximum-quantity' ) ?>"
        ...
        public function add_options() {
            ...
            if ( 'yit-plugins_page_yith-wc-min-max-qty' == get_current_screen()->id && ( isset( $_GET['tab'] ) && $_GET['tab'] == 'bulk' ) && ( isset( $_GET['section'] ) && $_GET['section'] == 'tags' ) && ( !isset( $_GET['action'] ) || ( $_GET['action'] != 'edit' && $_GET['action'] != 'insert' ) ) ) {
