    ...
    class YWMMQ_Products_Bulk_Ops {
        ...
        public function output() {
                ...
                'custom_columns'   => array(
                    ...
                    'column_override_qty' => function ( $item, $me ) {
                        ...
                        if ( $item['override_qty'] == 'yes' ) {
                            $class  = 'show';
                            $tip    = __( 'Yes', 'yith-woocommerce-minimum-maximum-quantity' );
                            $min    = get_post_meta( $item['ID'], '_ywmmq_product_minimum_quantity', true );
                            $max    = get_post_meta( $item['ID'], '_ywmmq_product_maximum_quantity', true );
            ...
            if ( !empty( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], basename( __FILE__ ) ) ) {
                ...
                if ( $item_valid !== true ) {
                    ...
                    foreach ( $product_ids as $product_id ) {
                        update_post_meta( $product_id, '_ywmmq_product_exclusion', $exclusion );
                        update_post_meta( $product_id, '_ywmmq_product_quantity_limit_override', $override_qty );
                        update_post_meta( $product_id, '_ywmmq_product_minimum_quantity', $minimum_qty );
                        update_post_meta( $product_id, '_ywmmq_product_maximum_quantity', $maximum_qty );
                    }
            ...
            if ( 'delete' === $table->current_action() ) {
                $message = sprintf( _n( '%s product removed successfully', '%s products removed successfully', count( $_GET['id'] ), 'yith-woocommerce-minimum-maximum-quantity' ), count( $_GET['id'] ) );
            }
            ...
            if ( isset( $_GET['id'] ) && !empty( $_GET['action'] ) && ( 'edit' == $_GET['action'] ) ) {
                ...
                $item = array(
                    'ID'           => $_GET['id'],
                    'excluded'     => get_post_meta( $_GET['id'], '_ywmmq_product_exclusion', true ),
                    'override_qty' => get_post_meta( $_GET['id'], '_ywmmq_product_quantity_limit_override', true ),
                    'minimum_qty'  => get_post_meta( $_GET['id'], '_ywmmq_product_minimum_quantity', true ),
                    'maximum_qty'  => get_post_meta( $_GET['id'], '_ywmmq_product_maximum_quantity', true )
                );
                            ...
                            <tr valign="top">
                                ...
                                <td class="forminp">
                                    ...
                                    <input
                                        ...
                                        id="product_ids"
                                        name="product_ids"
                                        data-placeholder="<?php _e( 'Search for a product&hellip;', 'yith-woocommerce-minimum-maximum-quantity' ) ?>"
        ...
        public function add_options() {
            ...
            if ( 'yit-plugins_page_yith-wc-min-max-qty' == get_current_screen()->id && ( isset( $_GET['tab'] ) && $_GET['tab'] == 'bulk' ) && ( !isset( $_GET['section'] ) || $_GET['section'] == 'products' ) && ( !isset( $_GET['action'] ) || ( $_GET['action'] != 'edit' && $_GET['action'] != 'insert' ) ) ) {
